...
class Reaction{
  ...
  costToMake(count: number): [number, Costs]{
    ...
    this.inputs.forEach((input) => {
      result[input.chemical] = 0
    })

    while(made < count){
      this.inputs.forEach((input) => {
        result[input.chemical] += input.qty
      })

      made += this.output.qty
    }

    const leftover = made - count
  ...
  plainCost(qty: number, costs: Costs){
    let factor = qty / this.output.qty

    this.inputs.forEach((chemical) => {
      if(!costs[chemical.chemical]){
        costs[chemical.chemical] = 0
      }

      costs[chemical.chemical] += (factor * chemical.qty)

      const reaction = getReactionForChemical(chemical.chemical)

      if(chemical.chemical !== 'ORE'){
        costs = reaction.plainCost(factor * chemical.qty, costs)
      }
    })
  ...
  totalInputCost(qty: number): [Costs, number]{
...
const [costs, ore] = FUEL.totalInputCost(1)
