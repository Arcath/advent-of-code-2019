...
class Wire{
  ...
  populate(){
    ...
    this.path.forEach((instruction) => {
      ...
      let count = parseInt(instruction.substr(1, instruction.length - 1))
  ...
  intersects(wire: Wire): Point[]{
    const intersects = []

    this.points.forEach((point) => {
      wire.points.forEach((secondPoint) => {
        if(point.equal(secondPoint)){
...
const Wire1 = new Wire(LINES[0])
const Wire2 = new Wire(LINES[1])

const Intersects = Wire1.intersects(Wire2)

const centrePoint = new Point(CENTRE, CENTRE)

const distance = Intersects.reduce((d, point) => {
  if(point.distance(centrePoint) < d || d === 0){
    return point.distance(centrePoint)
  }
